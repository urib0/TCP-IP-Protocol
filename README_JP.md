...
- 必要なパラメータ詳細：6
  ...
  | パラメータ名｜タイプ｜意味｜｜｜｜。
  | -------- | ------ | ------------ |
  | オフセットX｜double｜X軸のオフセット、mm単位｜。
  | offsetY｜double｜Y軸のオフセット（mm）｜。
  | オフセットZ｜double｜Z軸のオフセット（mm）｜。
  | offsetRx｜double｜Rx軸の位置、度数｜。
  | offsetRy｜double｜Ry軸の位置、度数｜。
  | offsetRz｜double｜Rz軸の位置（度数）｜。
...
- パラメータ詳細：10
  ...
  | パラメータ名｜タイプ｜意味｜｜｜。
  | -------- | ------ | ---------------------------------------- |
  | X｜double｜X軸の位置（ミリメートル単位）｜。
  | Y｜double｜Y軸の位置（ミリメートル単位）｜。
  | Z｜double｜Z軸の位置をミリメートル単位で表示します。
  | Rx｜double｜Rx軸の位置（°）｜。
  | Ry｜double｜回転軸の位置（°）｜。
  | Rz｜double｜Rz軸の位置（°）｜。
  | モード｜int｜距離のモードを設定します。0：距離は距離の割合、1：距離は開始点または目標点からの距離｜です。
  | Distance｜int｜指定された距離を実行：Modeが0の場合、Distanceは開始点から目標点までの距離の割合で、0から100の範囲です。Distanceが正の場合は、開始点からの距離、負の場合は、目標点からの距離です｜Distanceが0の場合は、開始点から目標点までの距離、正の場合は、開始点からの距離、負の場合は、目標点からの距離です。
  | インデックス｜int｜デジタル出力のインデックス、範囲：1〜24｜。
  | ステータス｜int｜出力状態を表す数値で、範囲は0または1｜です。
...
- 必要なパラメータ詳細：12
  ...
  | パラメータ名｜タイプ｜意味｜｜｜。
  | ---- | ------ | ------------------- |
  | X1｜double｜ 円弧の中点におけるX1軸の位置をミリメートル単位で示す。
  | Y1｜double｜ Y1軸上の円弧の中点の位置をミリメートル単位で示す。
  | Z1｜double｜ Z1軸上の円弧の中点の位置をミリメートル単位で示す。
  | Rx1｜double｜ Rx1軸上の円弧の中点の位置を度単位で示す。
  | Ry1｜double｜ Ry1軸上の円弧の中点の位置を度単位で示す。
  | Rz1｜double｜ 円弧の中点におけるRz1軸の位置を度単位で示す。
  | X2｜double｜ X2軸上の円弧の終点の位置をミリメートル単位で示す。
  | Y2｜double｜ 円弧の端のY2軸の位置をミリメートル単位で示す。
  | Z2｜double｜ 円弧の終点におけるZ2軸の位置をミリメートル単位で示す。
  | Rx2｜double｜ 円弧の終点におけるRx2軸の位置を度単位で示す。
  | Ry2｜double｜ 円弧の終点の Ry2 軸上の位置を度単位で示す。
  | Rz2｜double｜ 円弧の終点におけるRz2軸の位置を度単位で示す｜Rz2｜double｜円弧の終点におけるRz2軸の位置を度単位で示す。
...
- 必要なパラメータ詳細：13
  ...
  | ----- | ------ | ------------ |
  | カウント｜int｜整数値｜の数
  | X1｜double｜X1軸の位置、ミリメートル単位｜。
  | Y1｜double｜Y1軸の位置をミリメートル単位で表示します。
  | Z1｜double｜Z1軸の位置をミリメートル単位で表示します。
  | Rx1｜double｜Rx1軸の位置（度数）｜Rx2軸の位置（度数）｜Rx3軸の位置（度数
  | Ry1｜double｜Ry1軸の位置（度）｜Ry2軸の位置（度
  | Rz1｜double｜Rz1軸の位置（°）｜。
  | X2｜double｜X2軸の位置（mm）｜。
  | Y2｜double｜Y2軸の位置（mm単位）｜Y2軸の位置（mm単位）。
  | Z2｜double｜Z2軸の位置(mm)
  | Rx2｜double｜Rx2軸の位置（度）｜Rx2軸の位置（度
  | Ry2｜double｜ Ry2軸の位置（度数
  | Rz2｜double｜Rz2軸の位置（度）｜Rz2軸の位置（度
...
- 必須パラメータの詳細：6
  ...
  | パラメータ名｜タイプ｜意味｜｜｜。
  | ---- | ------ | --------------- |
  | J11｜double｜ポイントJ11軸の位置（度数）｜｜。
  | J12｜double｜点P1におけるJ12軸の位置（°）｜。
  | J13｜double｜P1点 J13軸の位置（度数）｜P2点 J13軸の位置（度数）｜P3点 J13軸の位置（度数
  | J14｜double｜P1点 J14軸の位置（度）｜P2点 J14軸の位置（度
  | J15｜double｜P1点 J15軸の位置（度）｜P2点 J15軸の位置（度
  | J16｜double｜P1 点 J16 軸の位置（度数）を指定します。
...
- パラメータ詳細：6
  ...
  | パラメータ名｜タイプ｜意味｜｜｜。
  | ---- | ------ | ------------ |
  | X1｜double｜X1軸の位置（mm）｜X2軸の位置（mm）｜X3軸の位置（mm
  | Y1｜double｜Y1軸の位置をミリメートル単位で表す｜。
  | Z1｜double｜Z1軸の位置をミリメートル単位で表示します。
  | Rx1｜double｜Rx1軸の位置（度数）｜Rx2軸の位置（度数）｜Rx3軸の位置（度数
  | Ry1｜double｜Ry1軸の位置（度）｜Ry2軸の位置（度
  | Rz1｜double｜Rz1軸の位置（°）｜。
...
- 必要なパラメータ詳細：7
  ...
  | -------- | ------ | -------------------- |
  | オフセットX｜double｜X軸のオフセット、mm単位｜。
  | オフセットY｜double｜Y軸のオフセット、mm単位｜。
  | オフセットZ｜double｜Z軸のオフセット、mm単位｜。
  | OffsetRx｜double｜Rx軸の位置（度）｜。
  | OffsetRy｜double｜Ry軸の位置（度数単位）｜緯度経度
  | OffsetRz｜double｜Rz軸の位置を度単位で指定します。
  | ツール｜int｜較正されたツールの座標系を0から9の範囲で選択します｜。
...
- 必要なパラメータ詳細：7
  ...
  | OffsetRy｜double｜Ry軸の位置（度数単位）｜緯度経度
  | OffsetRz｜double｜Rz軸の位置を度単位で指定します。
  | ユーザー｜int｜0から9の範囲で、較正されたユーザー座標系を選択します｜。
...
- 必須パラメータ詳細：6
  ...
  | パラメータ名｜タイプ｜意味｜｜｜。
  | ------- | ------ | ------------ |
  | Offset1｜double｜Offset1 のオフセット値（度数）｜。
  | オフセット2｜double｜オフセット2に対するオフセット値（度数単位）｜緯度経度
  | Offset3｜double｜Offset3 のオフセット値（度数）｜。
  | Offset4｜double｜Offset4 のオフセット値（度数）｜。
  | オフセット5｜double｜ノード5のオフセット角度
  | Offset6｜double｜Offset6 の度数単位のオフセット｜。
